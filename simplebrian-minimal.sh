...
echo
read -p "With that said, are you ready to install everything (MINIMAL INSTALLATION)? [Y/N] " -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]
then
# let the games begin.
...
sudo pacman -S --noconfirm --needed budgie-desktop budgie-extras lightdm bluez blueman bluez-utils tilix nemo gtk-engine-murrine gtk-engines pipewire plank vlc fuse2 fuse3 intel-ucode ufw neofetch gnome-system-monitor sassc gthumb gedit powerline-fonts sbctl base-devel git noto-fonts cups nss-mdns ghostscript xorg-server xorg-apps xorg-xinit xorg-twm xorg-xclock xterm bash-completion nano dhcpcd networkmanager
...
yay -S indicator-sysmonitor-budgie-git vala-panel-appmenu-budgie-git web-greeter spotify brave-bin p7zip-gui appimagelauncher libre-menu-editor --sudoloop --noconfirm
